/*
Program Description: Using merge sort orders the QA logs for each production line ion product id, issue code and date and time
...
int Merge_sort_task_2(struct QAlogs *,struct QAlogs *, int,int, int);
int Merge_task_2(struct QAlogs *, struct QAlogs *, int,int,int);
int binary_search(struct QAlogs *,struct QAlogs *, int,int, int,int,int);
...
Merge_sort(prod_1,temp1,low,high,mid);
...
Merge_sort(prod_2,temp1,low,high,mid);
...
Merge_sort(prod_3,temp1,low,high,mid);
...
Merge_sort(prod_4,temp1,low,high,mid);
...
Merge_sort_task_2(all_prod,temp2,low,high,mid);


printf("\n\n\t\t\t\t\t\tAll production lines:\n");

//Outputs the sorted log for all production lines
printf("Line ID  Batch Code  Day  Hour:Minutes  Product ID  Issue Code and Description  Resolution Code and Description  Reporting Employee ID\n");
for(i = 0; i < (amount*4); i ++)
{
    printf("%5.d", all_prod[i].line_code);
    printf("%10.d", all_prod[i].batch_code);
    printf("%8.d", all_prod[i].date_time.day);
    printf("%7.d:%.2d", all_prod[i].date_time.hour,all_prod[i].date_time.minute);
    printf("%12.d", all_prod[i].product_id);
    printf("%13.d: %-20s", all_prod[i].issue.code, all_prod[i].issue.description);
    printf("%8.d: %-15s\t", all_prod[i].resolution.code,all_prod[i].resolution.description);
    printf("%10.d\n", all_prod[i].employee_id);
}//End for
...
//Calls merge_sort function for all production lines
Merge_sort(all_prod,temp2,low,high,mid);

//Gets the product ID and issue code they are looking for
...
binary_search(all_prod,temp_search,low,high,mid,product_key,issue_key);

//Outputs the earliest occurrence of the issue code for that product ID.
printf("\n\n\t\t\t\tThe earliest occurrence of issue code %d for product id %d is:\n", issue_key,product_key);
printf("Line ID  Batch Code  Day  Hour:Minutes  Product ID  Issue Code and Description  Resolution Code and Description  Reporting Employee ID\n");
printf("%5.d", temp_search[0].line_code);
printf("%10.d", temp_search[0].batch_code);
printf("%8.d", temp_search[0].date_time.day);
printf("%7.d:%.2d", temp_search[0].date_time.hour,temp_search[0].date_time.minute);
printf("%12.d", temp_search[0].product_id);
printf("%13.d: %-20s", temp_search[0].issue.code, temp_search[0].issue.description);
printf("%8.d: %-15s\t", temp_search[0].resolution.code,temp_search[0].resolution.description);
printf("%10.d\n", temp_search[0].employee_id);
...
//Merge sort function
//Takes in the array of structures put's it in as a parameter when called and sets it to the pointer products
int Merge_sort_task_2(struct QAlogs *products, struct QAlogs *temp,int low,int high, int mid)
{   
    ...
    else
    {
        //Calculates the mid of the array of structures and rounds down
        mid = floor((low+high)/2);
        Merge_sort_task_2(products,temp,low,mid,mid);
        Merge_sort_task_2(products,temp,mid+1,high, mid);
        Merge_task_2(products,temp,low,mid,high);
...
//Merge function
//Puts the structures in the correct order of product ID,Issue Code and finally Date & time
int Merge_task_2(struct QAlogs *products,struct QAlogs *tempA,int low,int mid,int high)
{   
    int i;
    int left = low;
    int right = mid + 1;
...
{   
    if (low<=high)
    {
        ...
        {   
            ...
            {
                binary_search(products,search,mid-1,mid-1,mid-1,product_key,issue_key);
            }
        ...
        //If the product_id we are looking for is greater than or equal to the product ID in the list.
